...
Первым в списке стоит настроенная в [прошлой статье ВМ](https://ophil.ru/blog/02.fc41.html). Пока искал правильные опции для создания коннекшена (aka контекст в докере), столкнулся с подсказкой от подмана, мол, создайте сначала машину, а именно:
...
Уточню условия нашего эксперимента - мы настроили одинаковую среду разработки с помощью утилиты `mise`, см. [предыдущую статью](https://ophil.ru/blog/02.fc41.html), и на нашем дев. сервере, и у каждого из разрабов команды. Репозитарий git использует этот же дев. сервер, доступ к репо и серверу по ключу, парольный доступ закрыт. Пайплайны настроены как для локальной сборки, так и на дев. сервере. Перед запуском 3-й версии пайплайна на дев. сервере нужно сделать коммит изменений в репо - `buildah` не знает о коннекшенах, работает с кодом в текущем каталоге, см. строку 1: после логина на сервер сначала переключается в корень проекта. Предварительно выкачиваем образ компилятора go для сборки в контейнере - это вполне честно, мы же выкачали и настроили компилятор golang заранее. Замеряем:
...
Как я предположил в самом начале, мы попробовали вариант с гипотетической идеальной командой разрабов, работающей в Линукс, умеющей в make. А как быть обычному девопсу с разношерстой командой, где кто-то сидит под виндой, а кто-то ни за что не откажется от привычного Макбука с M4 процем?? Есть вариант и для этого случая. Пусть они пишут код и тестируют его как им нравится, а в нашем репо на дев. сервере мы сделаем хак #3, а именно **гит хук** и т.н. bare репозитарий, см. githooks(5), выполняющий наши цели сборки и старта приложения при коммите в репо.

Для этого нам на пару минут придётся стать безжалостным хакером, удаляющим лишнее, открывающим скрытые возможности гита. Выполняем следующие шаги:

1. заходим под юзером dev на сервер, создадим пустой каталог, напр., `mkdir -pv ~/bare/t0`. Это станет новым GIT_DIR, зайдём в него и выполним `cd ~/bare/t0;git init --bare`
...
... $ nl hooks/post-receive 
    ...
    14  done
```
Закомментированные строки 3, 8, 9 полезны при отладке пайплайна. Строки 4 и 5 задают, что есть собственно репозитарий, переменная GIT_DIR, и куда будут записываться файлы проекта, переменная WORK_TREE. В цикле от строки 6 до 14 читаются и обрабатываются 3 переменных, с которыми гит вызывает этот хук. Строка 11 принимает все изменения в репо и обновляет WORK_TREE, всё то, что гит обычно делает в общем каталоге, как видим, в bare репо они разные. Далее, в 12 строим имя лога и строка 13 - собственно пайплайн. 

3. идём в каталог где расположен репо проекта - без страха и сожаления удаляем старый и создаём новый под тем же именем: `cd ~/src;rm -rf traefik-golang;mkdir traefik-golang`

4. завершаем сессию на дев. сервере, возвращаемся на рабочий комп и заходим в репо проекта. Конечно же, репо цел, клоны репо не так просто уничтожить пока есть хотя бы одна копия. Теперь см. старые настройки `git remote -v` и их удаляем `git remote remove fc42dev` в моём случае. Создаём новый remote, указывая новый гит **bare** репо: `git remote add bare.t0 dev@fc42dev:~/bare/t0`. Это надо сделать также всем разрабам в их локальных копиях.

5. проверям результат, возможно, надо сделать новый комит и push в новый remote. СтОит посм. подробнее, как изменился репо проекта на сервере: проверить логи в ~/bare/t0/logs, сравнить конфиги обычного репо проекта и на сервере, проверить какие команды перестали работать в серверном репо. Например, в WORK_TREE не работают команды гит `status; branch; commit; log`. Т.е. наш хак #3 с `git --bare` не только позволил делать деплой на сервере, но также защитил репо от локальных изменений, серверный репо всегда в чистоте и порядке. Можно редактировать код, но закомитить его можно только через обычный репо, изменения на сервере удалятся после любого коммита. 
